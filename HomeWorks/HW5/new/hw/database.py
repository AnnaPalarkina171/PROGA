...
def text():
    for link in links:
        ...
        for div in divs:
            ...
            path = os.getcwd()
            lin = link.split('/')[-2]
            name1 = path + '\\' + 'book' + '\\' + lin[0:10] + '.txt'                        #text
            name2 = path + '\\' + 'book' + '\\' + lin[0:10] + '.xml'                        #mystem
            with open(name1, 'w', encoding='utf-') as f:
                f.write(clean_p)
            os.system(r'C:\\Users\\User\\Desktop\\mystem.exe -cdgin --format xml --eng-gr {} {}'.format(name1, name2))

def table():
    path = os.getcwd()
    conn = sqlite3.connect('mydatabase.db')
    cursor = conn.cursor()
    cursor.execute("""CREATE TABLE project(title text, data text, link text, text text, mystem text, lex text) """)
    for index in range(len(links)):
        ...
        name3 = path + '\\' + 'book' + '\\' + lin[0:11] + '.txt'
        with open(name1, 'r', encoding='utf-8') as f:
            text = f.read()
        with open(name2, 'r', encoding='utf-8') as t:
            mystem = t.read()
            with open(name3,'w',encoding='utf-8') as r:
                lexs = re.findall(r'lex="(.*?)"', mystem)
                for x in lexs:
                    r.write(x)
        with open(name3, 'r', encoding='utf-8') as r:
            lex = r.read()
        tup = (titles[index], data[index], links[index], text, mystem, lex)
        line.append(tup)
